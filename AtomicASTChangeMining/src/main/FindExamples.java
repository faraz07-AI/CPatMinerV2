...
//		findForStatments();
	...
	private static void findGaps() {
			...
			if (sizeDir.isDirectory()) {
				for (File patternDir : sizeDir.listFiles()) {
					...
					if (hasFor(oldChanges, oldLines) || hasFor(newChanges, newLines)) {
						...
						System.out.println(gaps + " " + gaps + " " + oldGaps + " " + newGaps);
					}
				}
			}
		}
		...
		FileIO.writeStringToFile(sb.toString(), "/Users/farazgurramkonda/IdeaProjects/CPatMinerV2/CPatMinerV2/AtomicASTChangeMining/pattern-gaps.txt");
	}
